...
from .modules.lhpf_decoder import LHPFDecoder
...
class PlanningModel(TorchModuleWrapper):
    def __init__(
        ...
        ref_free_traj=False,
        use_lhpf=False,
        lhpf_decoder_depth=1,
        lhpf_history_steps=2,
        freeze_pluto_backbone=False,
        ...
        self.lhpf_history_steps = lhpf_history_steps
        ...
        if self.use_lhpf:
            self.lhpf_decoder = LHPFDecoder(
                num_mode=num_modes,
                decoder_depth=lhpf_decoder_depth,
                dim=dim,
                num_heads=num_heads,
                mlp_ratio=4,
                dropout=dropout,
        ...
        if freeze_pluto_backbone:
            self.freeze_pluto_backbone()
    ...
    def forward(self, data):
        ...
        if ref_line_available:
            (
                ...
                data,
                {"enc_emb": x, "enc_key_padding_mask": key_padding_mask},
            )
            historical_planning_embedding = self._get_historical_planning_embedding(
                data
            )
            historical_valid_mask = self._get_historical_planning_valid_mask(data)
            if self.use_lhpf and historical_planning_embedding is not None:
                trajectory, probability, planning_embedding = self.lhpf_decoder(
                    planning_embedding=planning_embedding,
                    historical_planning_embedding=historical_planning_embedding,
                    enc_emb=x,
                    enc_key_padding_mask=key_padding_mask,
                    r_key_padding_mask=r_key_padding_mask,
                    m_pos=self.planning_decoder.m_pos,
                    historical_valid_mask=historical_valid_mask,
                )
        else:
            trajectory, probability = None, None
            planning_embedding = None
        ...
        if planning_embedding is not None:
            out["planning_embedding"] = planning_embedding
    ...
    def _get_historical_planning_embedding(self, data):
        if "historical_planning_embedding" in data:
            return data["historical_planning_embedding"]

        reference_line = data.get("reference_line", {})
        return reference_line.get("historical_planning_embedding")

    def _get_historical_planning_valid_mask(self, data):
        if "historical_planning_valid_mask" in data:
            return data["historical_planning_valid_mask"]

        reference_line = data.get("reference_line", {})
        return reference_line.get("historical_planning_valid_mask")

    def freeze_pluto_backbone(self):
        modules = [
            self.pos_emb,
            self.agent_encoder,
            self.map_encoder,
        ...
        if self.use_hidden_proj:
            modules.append(self.hidden_proj)
        if self.ref_free_traj:
            modules.append(self.ref_free_decoder)
