...
class PlutoPlanner(AbstractPlanner):
    ...
    def __init__(
        ...
        self._planning_embedding_history = deque(
            maxlen=getattr(planner, "lhpf_history_steps", 0)
        )
    ...
    def initialize(self, initialization: PlannerInitialization) -> None:
        ...
        if self._planner_ckpt is not None:
            strict = not getattr(self._planner, "use_lhpf", False)
            self._planner.load_state_dict(
                load_checkpoint(self._planner_ckpt),
                strict=strict,
            )

        self._planner.eval()
        self._planner = self._planner.to(self.device)
        self._planning_embedding_history.clear()
    ...
    def _run_planning_once(self, current_input: PlannerInput):
        ...
        self._attach_historical_planning_embedding(planner_feature_torch.data)
        out = self._planner.forward(planner_feature_torch.data)
        self._update_planning_embedding_history(out)
    ...
    def _attach_historical_planning_embedding(self, data):
        if not getattr(self._planner, "use_lhpf", False):
            return
        if len(self._planning_embedding_history) == 0:
            return

        _, num_ref, _, _ = data["reference_line"]["position"].shape
        num_modes = self._planner.num_modes
        valid_history = [
            embedding.to(self.device)
            for embedding in self._planning_embedding_history
            if embedding.shape[1] == num_ref and embedding.shape[2] == num_modes
        ]
        if len(valid_history) == 0:
            return

        data["reference_line"]["historical_planning_embedding"] = torch.stack(
            valid_history,
            dim=1,
        )

    def _update_planning_embedding_history(self, out):
        if not getattr(self._planner, "use_lhpf", False):
            return
        planning_embedding = out.get("planning_embedding")
        if planning_embedding is None:
            return
        self._planning_embedding_history.append(planning_embedding.detach().cpu())
